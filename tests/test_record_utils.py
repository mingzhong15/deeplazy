...
class TestOlpTask:
    def test_olp_task_creation(self):
        ...
        assert task.path == "/path/to/POSCAR"

    def test_olp_task_to_dict(self):
        task = OlpTask(path="/path/to/POSCAR")
        assert task.to_dict() == {"path": "/path/to/POSCAR"}

    def test_read_write_olp_tasks(self):
        tasks = [
            OlpTask(path="/path/a.vasp"),
            OlpTask(path="/path/b.vasp"),
        ]
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(loaded) == 2
            assert loaded[0].path == "/path/a.vasp"
            assert loaded[1].path == "/path/b.vasp"
...
class TestInferTask:
    ...
    def test_read_write_infer_tasks(self):
        tasks = [
            InferTask(path="/a.vasp", scf_path="batch.0/task.0/olp"),
            InferTask(path="/b.vasp", scf_path="batch.0/task.1/olp"),
        ]
        with tempfile.TemporaryDirectory() as tmpdir:
...
class TestCalcTask:
    ...
    def test_read_write_calc_tasks(self):
        tasks = [
            CalcTask(path="/a.vasp", geth_path="batch.0/task.0/infer"),
        ]
        with tempfile.TemporaryDirectory() as tmpdir:
