...
def test_version():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "deeplazy", "version"],
        capture_output=True,
        text=True,
    )
...
def test_help():
    """测试帮助命令"""
    result = subprocess.run(
        [sys.executable, "-m", "deeplazy", "--help"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
...
def test_validate_config():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "deeplazy", "validate", "--config", str(config_path)],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
...
def test_import():
    """测试导入"""
    from deeplazy import __version__, WorkflowExecutor
